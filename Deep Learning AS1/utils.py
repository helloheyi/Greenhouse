...
def load_mnist_trainval():
    ...
    data_Full = np.insert(data, 0, label, axis=1)
    ## length of training set 
    len_train = int(np.shape(data)[0]*0.8)
    ## train data 
    index_Full = np.arange(np.shape(data_Full)[0])
    selected_train_index = np.random.choice(index_Full, size=len_train, replace=False)
    train_data = (data_Full[selected_train_index][:,1:]).tolist()
    train_label_np = (data_Full[selected_train_index][:,0]).astype(int)
    train_label = list(map(int,train_label_np))
    selected_val_index = np.setdiff1d(index_Full, selected_train_index)
    val_data = (data_Full[selected_val_index][:,1:]).tolist()
    val_label_np = (data_Full[selected_val_index][:,0]).astype(int)
    val_label = list(map(int,val_label_np))
...
def generate_batched_data(data, label, batch_size=128, shuffle=False, seed=None):
    """
    Turn raw data into batched forms ( rather than passing the entire dataset through the neural network at once 
                                      (which is computationally expensive and memory-intensive), 
                                      the data is divided into smaller sets called batches)
    ...
    batched_data = []
    batched_label = []
    shuffled_data = data_Full[:,1:]
    shuffled_label = data_Full[:,0]

    for i in range(0, np.shape(shuffled_data)[0], batch_size):
    
    
        batched_data.append(np.array(shuffled_data[i:i + batch_size]))
        batched_label.append(np.array(shuffled_label[i:i + batch_size]))
    # batched_label_np = (batched_label).astype(int)
    # batched_label_F = []
    # for i in range(0,len(batched_label)):
    # # batched_label = list(map(int,batched_label))
    #     batched_label_F.append([int(element) for element in batched_label[i]])
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    x = range(0,len(train_loss_history))
    plt.plot(x,train_loss_history, label="Train", linestyle="-")
    plt.plot(x,valid_loss_history, label="Validation", linestyle="-")
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)    
    ...
    plt.plot(x,train_acc_history, label="Train", linestyle="-")
    plt.plot(x,valid_acc_history, label="Validation", linestyle="-")
    plt.xlabel("Epochs")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.grid(True)    
...
if __name__ == "__main__":
    ...
    batched_data, batched_label = generate_batched_data(train_data, train_label,
                                                        batch_size=1280, shuffle=True, seed=1024)
    
    # val_data, val_label = generate_batched_data(val_data, val_label,
    #                                                     batch_size=128, shuffle=True, seed=1024)
    
    
    
    # for i, b in enumerate(batched_data[:-1]):
    #     if len(b) !=128:
    #         print('b',len(b))
      
    #     if len(batched_data[i]) !=128:
    #         print('i', len(batched_data[i]))
