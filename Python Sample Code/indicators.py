...
def Boollinger_Bands(symbol,start_date,end_date,windown_size,figure= False):
    ...
    df['%B'] = (df[symbol]-df['Lower_Band'])/(df['Upper_Band']-df['Lower_Band'])
    df['Signal'] = 0
...
def Stochastic_KD(symbol,start_date,end_date,windown_size1,windown_size2,figure):
    ...
    if figure == True:
        ...
        ax[0].scatter(df[df['%D'] > 85].index, df[df['%K'] > 85][symbol], color='r', marker='^',
                   label='Sell Signal', s=20)
        ax[0].scatter(df[df['%D'] < 15].index, df[df['%K'] < 15][symbol], color='g', marker='o',
